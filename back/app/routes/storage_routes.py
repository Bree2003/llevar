...
def list_data_products_api():
    ...
    if not all([env_id, bucket_name]):
        raise InvalidUsage("Los parámetros 'env_id' y 'bucket_name' son requeridos.", status_code=400)
...
def list_subfolders_api(folder_path):
    ...
    try:
        project_id = get_project_id_for_bucket(env_id, bucket_name)
        subfolders = storage_service.list_subfolders_in_path(project_id, bucket_name, folder_path)
        return jsonify({"tables": subfolders})
    except InvalidUsage as e:
        raise e
    except Exception as e:
        raise InvalidUsage(f"Error al listar subcarpetas para la ruta '{folder_path}': {e}", status_code=500)
...
def get_latest_dataset_api(product_path):
    ...
    try:
        project_id = get_project_id_for_bucket(env_id, bucket_name)
        latest_dataset = storage_service.get_latest_dataset_in_product(project_id, bucket_name, product_path)
        return jsonify({"latest_dataset": latest_dataset})
    except InvalidUsage as e:
        raise e
    except Exception as e:
        raise InvalidUsage(f"Error al obtener el último dataset para '{product_path}': {e}", status_code=500)
...
def initiate_resumable_upload_api():
    ...
    if not origin_url:
        raise InvalidUsage("La cabecera 'Origin' es requerida.", status_code=400)
    ...
    try:
        ...
        logging_service.log_error("Fallo al iniciar sesión de subida", user=user, error=str(e))
...
def analyze_file_api():
    ...
    try:
        ...
        if step == "1":
            ...
            def map_dtype(dtype):
                if pd.api.types.is_numeric_dtype(dtype): return "Number"
                if pd.api.types.is_datetime64_any_dtype(dtype): return "Date"
                return "Text"
            ...
            if 'columna_esperada' not in df.columns:
                alerts.append("Alerta: La columna 'columna_esperada' no se encontró.")
...
def upload_file_api():
    ...
    if not all([env_id, bucket_name, destination]):
        raise InvalidUsage("Los campos 'env_id', 'bucket_name' y 'destination' son requeridos.", status_code=400)
    ...
    try:
        ...
        final_blob_path = storage_service.upload_file(project_id, bucket_name, parquet_path, destination)
...
def get_latest_dataset_preview_api(product_path):
    ...
    try:
        ...
        filename, df = storage_service.read_latest_dataset_preview(project_id, bucket_name, product_path)
        ...
        "exists": True,
        "fileName": filename,
        "columns": [c['nombre'] for c in columnas], # Lista simple de nombres para la tabla
        "data": vista_previa,
