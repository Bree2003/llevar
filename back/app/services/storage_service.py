...
def list_subfolders_in_path(project_id, bucket_name, path):
    """
    Lista las subcarpetas directas dentro de una ruta (path) específica en un bucket.
...
def get_latest_dataset_in_product(project_id, bucket_name, product_path):
    ...
    all_files = [blob for blob in blobs_iterator if not blob.name.endswith('/')]
...
def read_latest_dataset_preview(project_id, bucket_name, product_path, rows=20):
    """
    Busca el archivo más reciente, lee su contenido (Parquet o CSV) 
