...
export default function WizardModal({
  ...
  const hasTechnicalErrors = !isNewTable && currentStep === 3 && stepData?.bloqueantes?.length > 0;
  ...
  const hasMissingMetadata = isNewTable && currentStep === 3 && validateNewTableMetadata();

  // El botón se bloquea si: subiendo, cargando, errores técnicos o faltan metadatos
  const isButtonDisabled = isUploading || isLoading || hasTechnicalErrors || hasMissingMetadata;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md text-center animate-fadeIn">
          ...
          <p className="text-gray-600 mb-6">El archivo ha sido procesado y cargado correctamente en BigQuery. Tabla {uploadMessage.b_query}</p>
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md text-center animate-fadeIn">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Error en la Ingesta</h2>
          <p className="text-gray-600 mb-6 text-sm">
            Hubo un problema procesando el archivo:<br/>
            <span className="font-mono bg-red-50 text-red-700 p-1 rounded mt-2 block">{uploadError}</span>
          </p>
          ...
          <div className="flex justify-center items-center gap-4">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center">
                ...
                {s < 3 && (
                  <div className={`w-12 h-1 mx-2 transition-colors ${s < currentStep ? "bg-green-500" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
            ...
            {currentStep < 3 ? (
              ...
              <div className="flex flex-col items-end">
                <button
                  ...
                  title={hasMissingMetadata ? "Completa todas las descripciones para continuar" : ""}
                >
                  {isUploading
