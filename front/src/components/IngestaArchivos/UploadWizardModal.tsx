...
export default function UploadWizardModal({ isOpen, onClose, initialData, file }: ModalProps) {
...
const [stepData, setStepData] = useState<{ [key: number]: any }>({});
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-2xl p-6 md:p-8 w-full max-w-3xl">
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Carga de Archivos</h2>
                ...
                <div className="flex justify-between items-center mt-6">
                    ...
                    <div className="flex gap-3">
                        {currentStep > 1 && (
                            <button onClick={handlePrevious} disabled={isLoading} className="rounded-md bg-gray-200 py-2 px-6 text-sm font-medium text-gray-800 hover:bg-gray-300 disabled:opacity-50">
                                Anterior
                            </button>
                        )}
                        {currentStep < 3 ? (
                            <button onClick={handleNext} disabled={isLoading} className="rounded-md bg-[#F46546] py-2 px-6 text-sm font-medium text-white shadow-sm hover:bg-opacity-90 disabled:bg-gray-400">
                                {isLoading ? 'Cargando...' : 'Siguiente'}
                            </button>
                        ) : (
                            <button onClick={handleFinalIngest} disabled={isLoading} className="rounded-md bg-red-600 py-2 px-6 text-sm font-medium text-white shadow-sm hover:bg-red-700 disabled:bg-gray-400">
                                {isLoading ? 'Procesando Ingesta...' : 'Confirmar Ingesta'}
                            </button>
                        )}
                    </div>
                </div>
                {finalMessage && <div className="mt-4 text-center text-sm p-2 bg-gray-100 rounded">{finalMessage}</div>}
            </div>
        </div>
    );
}